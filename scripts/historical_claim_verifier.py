#!/usr/bin/env python3
"""SWI Historical Claim Verifier v2 — strict E4, fail-closed. See docs/V47_VERIFIER_HARDENING.md"""
...
def normalize_module(raw: str) -> Optional[str]:
    ...
    if not raw.startswith("M"): raw = "M" + raw
    m = MODULE_ID_RE.match(raw)
    if not m: return None
    n = int(m.group(1))
    if n < 0 or n > MAX_MODULE: return None
    return f"M{n:02d}"

def classify_path(path: str) -> str:
    p = path.replace("\\", "/").lower()
    if p.startswith("docs/") or p.endswith(".md"): return "DOCUMENTATION"
    if "/test/" in f"/{p}" or p.startswith("test/"): return "TEST"
    if ".github/workflows/" in p: return "WORKFLOW"
    if p.endswith(".json") and ("audit" in p or "evidence" in p): return "ARTIFACT"
    if p.endswith(".py"): return "SOURCE"
    return "OTHER"

def is_stub_source(path: str, size: Optional[int] = None) -> bool:
    if re.search(r"module\\d+/__init__\\.py$", path.replace("\\", "/")):
...
def parse_audit_json_strict(text: str, module: str) -> Tuple[bool, Dict[str, Any]]:
    ...
    except json.JSONDecodeError: return False, {"error": "invalid_json"}
    if not isinstance(data, dict): return False, {"error": "not_object"}
    if normalize_module(str(data.get("module", ""))) != normalize_module(module): return False, {"error": "module_mismatch"}
    if str(data.get("status", "")).upper() != "SEALED": return False, {"error": "status_not_sealed"}
...
def build_report(*, tree, modules, v2_commit, audit_json_text, seal_md_text, ci_run_id, ci_verified, ci_self_attested, package_status):
    ...
    for mod in modules:
        ...
        if mod == "M11":
            ...
            if seal_md_text is not None:
                ...
                if not seal.ok: ev.flags.append("SEAL_UNVERIFIED")
            else:
            ...
            if audit_json_text is not None:
                ...
                if aok and seal_md_text is not None:
                    ...
                    if seal.ci_run and adata.get("workflow_run") and seal.ci_run != str(adata.get("workflow_run")):
                        ev.flags.append("AUDIT_SEAL_CI_CONFLICT"); conflicts.append("audit run ≠ seal run")
            if ci_verified and ci_run_id:
                ev.ci_verified = True; ev.ci_run_id = ci_run_id; ev.ci_self_attested = ci_self_attested
            else:
    ...
    return {"audit_id": f"V47-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}", "verifier_version": "2-strict-e4",
            "package_status": package_status,
            "repositories": {"V2": {"repository": "Kelronmos/SWI-V2-Modules-11-22", "commit": v2_commit or "UNVERIFIED"}},
            "retrieved_at": datetime.now(timezone.utc).isoformat(), "modules": results, "conflicts": conflicts,
...
def demo_m11_regression():
    paths = ["swi_v2/kernel/seal.py","swi_v2/kernel/admission.py","swi_v2/kernel/ed25519_sig.py","swi_v2/kernel/merkle.py",
    ...
    return build_report(tree=tree, modules=["M11","M12"], v2_commit="1d6d7dc250df80f39aa60bd8da812c9ae3efebec",
...
def demo_fake_seal_not_e4():
    ...
    report = build_report(tree=tree, modules=["M11"], v2_commit="1d6d7dc250df80f39aa60bd8da812c9ae3efebec",
...
def demo_audit_alone_not_e4():
    ...
    report = build_report(tree=tree, modules=["M11"], v2_commit="1d6d7dc...", audit_json_text=audit, seal_md_text=None,
                          ci_run_id=None, ci_verified=False, ci_self_attested=False, package_status="STANDALONE_DOCUMENTS")
    assert report["modules"][0]["evidence_level"] != "E4_SEALED"
...
def main() -> int:
    ...
    if args.package_status == "INCOMPLETE":
        print("PACKAGE_STATUS=INCOMPLETE", file=sys.stderr); return 2
    if args.demo_backtests:
        demo_fake_seal_not_e4(); demo_audit_alone_not_e4(); return 0
    if args.demo_m11_regression:
        report = demo_m11_regression()
    else:
        if not args.v2_tree or not args.v2_tree.exists():
            print("Need --v2-tree or --demo-m11-regression", file=sys.stderr); return 2
        tree = json.loads(args.v2_tree.read_text())
        if isinstance(tree, dict) and "tree" in tree: tree = tree["tree"]
        self_attested = bool(args.ci_verified) and not bool(args.ci_independent)
        report = build_report(tree=tree, modules=[normalize_module(x) or x.strip().upper() for x in args.modules.split(",")],
                              ...
                              ci_self_attested=self_attested, package_status=args.package_status)
